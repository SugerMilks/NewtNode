...
- Keep My Newt Economy and Smart Text routing independent from Director/Storyboard intelligence. Image-model choice, resolution and fidelity do not change with an LLM upgrade.
...
- My Newt has image, video, audio, Character, and Mood Board inputs. Keep legacy Character/Mood Board connections to imageIn compatible. Snapshots include active sheet selections, generated/CU/custom sheets, and mood-board images using full-resolution sources.
...
- Only one My Newt exists per project. Add focuses the existing node, paste omits duplicates, import detaches task IDs, and graph normalization removes duplicate agents and dangling edges.
...
- `recovery.js` captures task-start creative graph checkpoints excluding My Newt and secrets. Follow-ups inherit relevant context and created-node access with a new budget. Restoration requires confirmation and no active operation; preserve current My Newt state, generated files, and billing history. Failed note/voice sends preserve the draft until acknowledged.
